...
async function fetchAIPicks(sport, betType = "all") {
  const cacheKey = `sage_v3_${sport}_${betType}_${new Date().toDateString()}`;
  try {
  ...
  try {
    const res = await fetch("/api/picks", {
      ...
      body: JSON.stringify({ sport, betType })
    });
    const data = await res.json();
    const picks = data.picks || [];
    if (picks.length > 0) {
      try { sessionStorage.setItem(cacheKey, JSON.stringify(picks)); } catch(e) {}
    }
    return picks;
  } catch(e) {
    console.error("fetchAIPicks error:", e);
    return [];
  }
}
...
async function generateAccumulator(sport, count, betPreference) {
  try {
    const res = await fetch("/api/picks", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sport, betType: betPreference, count })
    });
    const data = await res.json();
    const legs = (data.picks || []).slice(0, count).map(p => ({
      id: p.id,
      match: `${p.away} vs ${p.home}`,
      league: p.league,
      bet: p.bet,
      odds: parseFloat(p.odds) || 1.8,
      confidence: p.confidence,
      kickoff: p.kickoff,
      reasoning: p.reasoning?.split(".")[0] + "."
    }));
    if (!legs.length) return null;
    const totalOdds = legs.reduce((acc, l) => acc * l.odds, 1);
    const overallConf = Math.round(legs.reduce((acc, l) => acc + l.confidence, 0) / legs.length);
    return { legs, totalOdds, overallConf, sageTip: `${count}-leg ${sport} acca with avg ${overallConf}% confidence` };
  } catch(e) { return null; }
}
